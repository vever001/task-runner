...
if (!isset($classLoader)) {
  print "Unable to find autoloader for Task Runner\n";
  exit(1);
}

require_once __DIR__ . '/run-robo-run.php';
...
function find_repo_root() {
  ...
  foreach ($possible_repo_roots as $possible_repo_root) {
    if ($repo_root = find_directory_containing_files($possible_repo_root, ['vendor/bin/run', 'vendor/autoload.php'])) {
      return $repo_root;
    }
  }
}
...
function find_directory_containing_files($working_directory, array $files, $max_height = 10) {
  // Find the root directory of the git repository containing Task Runner.
